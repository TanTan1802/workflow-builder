...
# Serve static files (tùy chọn)
npx serve dist/workflow-builder/browser
```
...
Project này đã được cấu hình với GitHub Actions pipeline tự động:

### Pipeline Features
- ✅ **Automated Testing**: Unit tests với Karma/Jasmine
- 🔍 **Security Scanning**: Trivy vulnerability scanner
- 🐳 **Docker Build**: Multi-platform images (amd64/arm64)
- 📦 **Container Registry**: Tự động push đến GitHub Container Registry
- 🚀 **GitHub Pages**: Deploy tự động cho master branch
- 📊 **Build Artifacts**: Upload build outputs

### Workflow Triggers
- Push đến `master` và `develop` branches
- Pull requests đến `master` branch

### Container Registry
Images được tự động push đến:
```
ghcr.io/tantan1802/workflow-builder:latest
ghcr.io/tantan1802/workflow-builder:sha-[commit]
```

### Deploy với Container
```bash
# Pull latest image
docker pull ghcr.io/tantan1802/workflow-builder:latest

# Run container
docker run -d -p 8080:80 ghcr.io/tantan1802/workflow-builder:latest
```
# Build ứng dụng
npm run build
